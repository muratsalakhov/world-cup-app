...
$count = 1;
$query = "SELECT * FROM players WHERE player_passes>1 ORDER BY player_passes DESC, player_games, player_minutes LIMIT 50";
...
<div class="goals-section">
	<div class="top-heading">
		<h2>Отдали больше всего передач</h2>
	</div>
	...
	<?php 
		foreach ($players as $player) {?>
			<div class="player-row">
				...
				if($count==2) { silver(); }
				if($count==3) { bronze(); }?>>
					<p><?php echo $player['player_passes'];
					$count++;?></p>
				</div>
			</div>
		<?php } ?>
